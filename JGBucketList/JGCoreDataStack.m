...
{
    // we want to make sure that only one instance of or JGCoreDataStack exists. So we create a static variable - a static variable will exist only once in an applicatioin.
    static JGCoreDataStack *defaultStack;
    // Wnat to make sure that when we initalize this default stack it only happens once. We use grand central dispactches dispatch_once_t method to do so. Whatever code you put in there, it's guarantted to only be executed once
