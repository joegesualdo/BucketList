...
// The code below is stock code that will be almost always used with the fetch cotroller.

//Animations in tableviews use a batch approach, where we begin the updates, perform the updates that aren't reflected immediately, and then end the update, and then all the updates are reflected at once with animation
// To do this animation we implement these 4 methods, instead of just doing controller:didChangeChangeContent and reloading the tableview
...
{
    switch (type) {
        // If the type is NSFetchedResultsChangeInsert, the method insertRowAtIndexPath method will be called on the table view. This will insert a new row at the specified indexPath. This will do it with animation automatic
...
// It's called whenver a section is inserted or deleted. So when we delete the last row of a section, this is called. Or when we insert the first row of a section this is called
...
-(void)controllerDidChangeContent:(NSFetchedResultsController *)controller
{
    ...
    // We are commenting it out, because we want to add animation so we implemetn both controllerWillChangeContent also
